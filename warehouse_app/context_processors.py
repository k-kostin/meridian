...
def app_context(request):
    ...
    local_mode = settings.DEMO_MODE or settings.LOCAL_TRUSTED_MODE
    role_display = role_label(role) if request.user.is_authenticated else ("Локальный режим" if local_mode else "Гость")
    return {
