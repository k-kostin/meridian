...
- core operation user attribution для документов движения, инвентаризаций и событий проведения:
...
- экран предпросмотра импорта номенклатуры:
  ...
  - распространенные алиасы колонок для Excel-файлов;
  - нормализация реалистичных заголовков вроде `Код товара`, `Наименование товара`, `Ед. изм.`;
  - явный режим обновления существующих позиций без изменения поведения по умолчанию;
...
- category-aware операционные фильтры:
  - текущие остатки;
  - подневный отчет;
  - помесячный отчет;
  - аналитика периода;
  - соответствующие Excel-выгрузки;
- пользовательские сохраненные представления:
  - для журнала документов движения;
  - для текущих остатков;
  - с владельцем и сохраненными query-параметрами;
- импорт стартовых остатков:
  - загрузка `.xlsx` с листом `Стартовые остатки`, `Остатки`, `Остатки склада`, `Opening stock` или активным листом;
  - показ preview-строк и row-level validation errors;
