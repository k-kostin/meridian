...
- проверяет план миграций и создает `pre_migration` backup только перед реальным обновлением существующей SQLite-базы;
- не создает автоматическую копию для новой или уже актуальной базы;
- после успешной миграции сохраняет пять последних автоматических копий и никогда не удаляет ручные копии;
...
- `WAREHOUSE_PRE_MIGRATION_BACKUP_RETENTION=<N>` — число сохраняемых автоматических копий, по умолчанию `5`.

Если `pre_migration` backup не удалось создать, sidecar должен остановить запуск вместо миграции незащищенной локальной базы.
